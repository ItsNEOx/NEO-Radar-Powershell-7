...
function Start-NeoRadar {
    ...
    if ($Mode -eq 7) {
        ...
        Write-Host "   and scans key management ports. OS confidence: very high / high /" -ForegroundColor Gray
        Write-Host "   medium / low based on TTL range accuracy." -ForegroundColor Gray
        Write-Host ""
        ...
        Write-Host "   Multi-signal OS detection scoring based on TTL, open ports, hostname," -ForegroundColor Gray
        Write-Host "   HTTP server header, NetBIOS name, and MAC OUI. Confidence level: very" -ForegroundColor Gray
    ...
    if ($Mode -eq 9) {
        ...
        Write-Host "   OS confidence: very high when TTL matches a known range" -ForegroundColor Gray
        Write-Host "   (Windows 100-128, Linux/Unix 32-64, Network 240-255)." -ForegroundColor Gray
        ...
        Write-Host "5) Hostname Scan & Fingerprinting" -ForegroundColor Yellow
        Write-Host "   Neo-Radar: Multi-signal OS detection with confidence rating." -ForegroundColor Gray
        Write-Host "   Signals: TTL, open ports, hostname, HTTP server header," -ForegroundColor Gray
        Write-Host "   NetBIOS name, and MAC OUI vendor. Each signal awards points" -ForegroundColor Gray
        Write-Host "   to Windows / Linux / Apple / Network buckets. The leading" -ForegroundColor Gray
        Write-Host "   bucket's share of total points determines confidence:" -ForegroundColor Gray
        Write-Host "   ≥80% very high, ≥60% high, ≥40% medium, <40% low." -ForegroundColor Gray
    ...
    if ($Mode -eq 2) {
        ...
        $pct = 100
        $confidence = if ($ttl -ge 32 -and $ttl -le 255) { "very high" } else { "medium" }
        if ($os -match "^Apple") { $confidence = "very high" }
    ...
    if ($Mode -eq 5) {
        ...
        $HostScript = {
            ...
            $keyPorts = @(22, 23, 80, 135, 139, 443, 445, 548, 1433, 3389, 3689, 5000, 5555, 5900, 62078, 8080)
            $openPorts = @()
            ...
            # TTL
            if ($ttl -ge 100 -and $ttl -le 128) { $scores.Windows += 3; $signals += "TTL:$ttl(Win+3)" }
            elseif ($ttl -ge 32 -and $ttl -le 64) { $scores.Linux += 3; $signals += "TTL:$ttl(Lin+3)" }
            elseif ($ttl -ge 240 -and $ttl -le 255) { $scores.Network += 3; $signals += "TTL:$ttl(Net+3)" }
            else { $signals += "TTL:$ttl" }

            # Hostname clues
            if ($hostName) {
                $h = $hostName.ToUpper()
                if ($h -match 'IPHONE|IPAD|IPOD') { $scores.Apple += 5; $signals += "Host:Apple(+5)" }
                elseif ($h -match 'MACBOOK|MAC-PRO|IMAC|MACMINI|APPLE') { $scores.Apple += 5; $signals += "Host:Apple(+5)" }
                elseif ($h -match 'ANDROID|SM-|GT-|REDMI|POCO|MOTO|ONEPLUS') { $scores.Linux += 4; $signals += "Host:Android(+4)" }
                elseif ($h -match 'WIN-|DESKTOP-|LAPTOP-|WORKSTATION') { $scores.Windows += 4; $signals += "Host:Windows(+4)" }
                elseif ($h -match 'ROUTER|GATEWAY|SWITCH|AP-|OPENWRT|DD-WRT') { $scores.Network += 5; $signals += "Host:Network(+5)" }
            }

            # Port evidence
            if ($openPorts -contains 445)  { $scores.Windows += 5; $signals += "SMB(+5)" }
            if ($openPorts -contains 135)  { $scores.Windows += 3; $signals += "RPC(+3)" }
            if ($openPorts -contains 139)  { $scores.Windows += 3; $signals += "NetBIOS(+3)" }
            if ($openPorts -contains 3389) { $scores.Windows += 4; $signals += "RDP(+4)" }
            if ($openPorts -contains 1433) { $scores.Windows += 3; $signals += "MSSQL(+3)" }
            if ($openPorts -contains 22)   { $scores.Linux += 3; $signals += "SSH(+3)" }
            if ($openPorts -contains 5555) { $scores.Linux += 4; $signals += "ADB(+4)" }
            if ($openPorts -contains 5900) { $scores.Linux += 2; $signals += "VNC(+2)" }
            if ($openPorts -contains 23)   { $scores.Network += 3; $signals += "Telnet(+3)" }
            if ($openPorts -contains 8080) { $scores.Network += 1; $signals += "Alt-HTTP(+1)" }
            if ($openPorts -contains 5000)  { $scores.Apple += 4; $signals += "AirPlay(+4)" }
            if ($openPorts -contains 62078) { $scores.Apple += 5; $signals += "iTunes(+5)" }
            if ($openPorts -contains 548)   { $scores.Apple += 4; $signals += "AFP(+4)" }
            if ($openPorts -contains 3689)  { $scores.Apple += 3; $signals += "DAAP(+3)" }

            # HTTP server header
            if ($serverHeader) {
                if ($serverHeader -match "Microsoft-IIS") { $scores.Windows += 4; $signals += "IIS(+4)" }
                elseif ($serverHeader -match "Apache|nginx|lighttpd") { $scores.Linux += 2; $signals += "Web:Linux(+2)" }
                elseif ($serverHeader -match "micro_httpd|GoAhead|RomPager|Boa|httpd") { $scores.Network += 4; $signals += "IoT-srv(+4)" }
                elseif ($serverHeader -match "AirHttpd|CUPS") { $scores.Apple += 4; $signals += "Apple-srv(+4)" }
            }

            # NetBIOS name is strongly Windows
            if ($netbiosName) { $scores.Windows += 4; $signals += "NetBIOS(+4)" }
            ...
            $portHint = ""
            $serviceLabels = @{ 22 = "SSH"; 23 = "Telnet"; 80 = "HTTP"; 135 = "RPC"; 139 = "NetBIOS"; 443 = "HTTPS"; 445 = "SMB"; 548 = "AFP"; 1433 = "MS-SQL"; 3389 = "RDP"; 3689 = "DAAP"; 5000 = "AirPlay"; 5555 = "ADB"; 5900 = "VNC"; 62078 = "iTunes"; 8080 = "Alt-HTTP" }
            if ($openPorts.Count -gt 0) {
                $labels = $openPorts | ForEach-Object { if ($serviceLabels.ContainsKey($_)) { $serviceLabels[$_] } else { "$_" } }
                $portHint = " [" + ($labels -join ", ") + "]"
            }

            $signalStr = if ($signals.Count -gt 0) { " {" + ($signals -join ", ") + "}" } else { "" }
        ...
        try {
            ...
            foreach ($res in $sortedHostResults) {
                ...
                Write-Host "  $($res.IP) -> Host: $($res.Hostname) | OS: $($res.OS) (confidence: $($res.Confidence))$($res.PortSignals)$($res.SignalDetail)" -ForegroundColor $color
                $scanLog += "  $($res.IP) -> Host: $($res.Hostname) | OS: $($res.OS) (confidence: $($res.Confidence))$($res.PortSignals)$($res.SignalDetail)"
            }
            $scanLog += "============================================"
